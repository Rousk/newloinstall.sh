...
CAUTION! Run only as root
ВНИМАНИЕ! Запуск программы только от root.
Требуется запускать инструмент только от root
для его коректной работы.
Если скрипт был запущен не от root, сделайте отмену и перезапустите скрипт от root.
Общая информация:
